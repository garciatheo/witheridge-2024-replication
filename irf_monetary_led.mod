...
//
//  FIGURE 5 - Monetary-led regime: IRFs to a monetary policy shock.
...
beta    = 0.99;
omega   = 0.3;
phi_pi  = 1.5;        // monetary-led: strong central-bank response to inflation
gamma_d = 1.5;        // monetary-led: strong fiscal response to debt
gamma_y = 0;
rho_m   = 0.5;
sigma_m = 0.01;
...
// paper's Figure 5. It carries no economic content. (We do NOT rescale the
// IRFs by r's impact, because in the monetary-led regime r's impact response
// is tiny and negative, which would flip and blow up every series.)
shocks;
var em; stderr 600;
end;
